...
class TestCLI:
    def test_domains_command(self, tmp_path):
        ...
        from mempalace.cynefin_cli import cmd_domains, _build_parser
    ...
    def test_list_empty(self, tmp_path):
        ...
        from mempalace.cynefin_cli import cmd_list, _build_parser

        db = CynefinDB(db_path=str(tmp_path / "cli_test.db"))
        parser = _build_parser()
        args = parser.parse_args(["list"])
        out = StringIO()
        with patch("sys.stdout", out), patch("mempalace.cynefin_cli._db", return_value=db):
            cmd_list(args)
    ...
    def test_new_and_show(self, tmp_path):
        ...
        from mempalace.cynefin_cli import cmd_new, cmd_show, _build_parser
        ...
        out2 = StringIO()
        with patch("sys.stdout", out2), patch("mempalace.cynefin_cli._db", return_value=db):
            cmd_show(args_show)
    ...
    def test_import_command(self, tmp_path):
        ...
        from mempalace.cynefin_cli import cmd_import, _build_parser

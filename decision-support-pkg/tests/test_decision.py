...
from decisionsupport.decision import FRAMEWORKS, get_framework, set_framework, toggle
...
class TestDecisionCLI:
    def _run(self, argv, cfg_path, capsys):
        ...
        ), patch("decisionsupport.decision._CONFIG_PATH", cfg_path), patch(
            "decisionsupport.decision_cli.get_framework",
            lambda config_path=None: get_framework(cfg_path),
        ), patch(
            "decisionsupport.decision_cli.set_framework",
            lambda fw, config_path=None: set_framework(fw, cfg_path),
        ), patch(
            "decisionsupport.decision_cli.toggle",
            lambda config_path=None: toggle(cfg_path),
        ):
            from decisionsupport.decision_cli import main
